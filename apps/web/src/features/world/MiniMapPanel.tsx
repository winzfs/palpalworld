...
export function MiniMapPanel({
  ...
  return (
    <div className="minimap-panel minimap-panel--single-map">
      ...
      <div className="minimap-legend">
        ...
        <span><i className="minimap-dot minimap-dot--resource" />자원 {resources.length}</span>
        <span><i className="minimap-dot minimap-dot--creature" />몬스터 {creatures.length}</span>
        <span><i className="minimap-dot minimap-dot--building" />건물 {buildings.length}</span>
      </div>
      ...
      <style>{`
        ...
        .minimap-legend { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 6px; color: rgba(226,232,240,.82); font-size: 10px; }
        ...
        @media (max-width: 720px) { .minimap-map { min-height: 150px; } .minimap-legend { grid-template-columns: 1fr; } }
      `}</style>
    </div>
  );
}
