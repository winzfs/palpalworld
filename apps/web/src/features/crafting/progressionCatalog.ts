...
export const CRAFTING_STATIONS: CraftingStationDefinition[] = [
  { id: "hand", name: "손 제작", buildingType: null, description: "건설물 없이 바로 가능한 초반 제작입니다.", queueSize: 1 },
  { id: "workbench", name: "작업대", buildingType: "workbench", description: "도구, 초급 장비, 포획구, 기본 건설 키트를 제작합니다.", queueSize: 1 },
  ...
  { id: "advanced_workbench", name: "고급 작업대", buildingType: "advanced_workbench", description: "중급 장비, 개량 포획구, 고급 건설 키트를 제작합니다.", queueSize: 2 },
...
export const PROGRESSION_RECIPES: ProgressionRecipe[] = [
  { id: "workbench_kit", name: "작업대 키트", tier: "초반", station: "hand", category: "building", inputs: [{ itemId: "wood", amount: 20 }, { itemId: "stone", amount: 8 }], outputs: [{ itemId: "workbench_kit", amount: 1 }], craftTimeMs: 1500, description: "첫 제작소를 설치하기 위한 기본 키트." },
  { id: "base_core_kit", name: "거점 코어 키트", tier: "초반", station: "workbench", category: "building", inputs: [{ itemId: "wood", amount: 40 }, { itemId: "stone", amount: 30 }, { itemId: "pal_essence", amount: 1 }], outputs: [{ itemId: "base_core_kit", amount: 1 }], craftTimeMs: 5000, description: "거점 중심 구조물을 설치하는 핵심 부품." },
...
export function getBuildingKitCraftingStation(building: ProgressionBuilding): CraftingStationId {
  if (building.type === "workbench") return "hand";
  if (building.tier === "초반" || building.tier === "초중반") return "workbench";
  if (building.tier === "중반") return "advanced_workbench";
  return "assembly_bench";
}

export function getBuildableBuildingsByStation(stationId: CraftingStationId) {
  return PROGRESSION_BUILDINGS.filter((building) => getBuildingKitCraftingStation(building) === stationId);
}
