...
export class PrimitiveRenderer {
  ...
  drawCreature(ctx: CanvasRenderingContext2D, creature: CreaturePublicState, x: number, y: number) {
    ...
    this.drawShadow(ctx, x, y + 15, 34 + Math.abs(breathe), 10);

    switch (creature.speciesId) {
      case "leafbun":
        this.drawLeafbun(ctx, x, drawY, fill, wiggle, breathe);
        break;
      case "droplet":
        this.drawDroplet(ctx, x, drawY, wiggle, breathe);
        break;
      case "sparkit":
        this.drawSparkit(ctx, x, drawY, wiggle, breathe, time);
        break;
      case "rockturtle":
        this.drawRockturtle(ctx, x, drawY, wiggle, breathe);
        break;
      case "moleminer":
        this.drawMoleminer(ctx, x, drawY, wiggle, breathe);
        break;
      case "mossboar":
        this.drawMossboar(ctx, x, drawY, wiggle, breathe);
        break;
      default:
        this.drawGenericCreature(ctx, x, drawY, fill, breathe);
        break;
    }
  ...
  private drawLeafbun(ctx: CanvasRenderingContext2D, x: number, y: number, fill: string, wiggle: number, breathe: number) {
    ctx.strokeStyle = "#1f1308";
    ctx.lineWidth = 3;
    ctx.fillStyle = "#86efac";
    ctx.beginPath();
    ctx.ellipse(x - 9 + wiggle * 0.2, y - 20, 7, 16 + breathe * 0.2, -0.45, 0, Math.PI * 2);
    ctx.ellipse(x + 9 + wiggle * 0.2, y - 20, 7, 16 + breathe * 0.2, 0.45, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = fill;
    ctx.beginPath();
    ctx.roundRect(x - 17, y - 13, 34, 30 + breathe, 12);
    ctx.fill();
    ctx.stroke();
    this.drawEyes(ctx, x, y - 2);
  ...
  private drawDroplet(ctx: CanvasRenderingContext2D, x: number, y: number, wiggle: number, breathe: number) {
    ctx.strokeStyle = "#075985";
    ctx.lineWidth = 3;
    ctx.fillStyle = "rgba(56, 189, 248, 0.9)";
    ctx.beginPath();
    ctx.moveTo(x, y - 24 - breathe);
    ctx.bezierCurveTo(x + 24 + wiggle, y - 2, x + 15, y + 18, x, y + 18);
    ctx.bezierCurveTo(x - 15, y + 18, x - 24 + wiggle, y - 2, x, y - 24 - breathe);
    ctx.fill();
    ctx.stroke();
    ...
    ctx.fill();
    this.drawEyes(ctx, x, y - 1);
  }

  private drawSparkit(ctx: CanvasRenderingContext2D, x: number, y: number, wiggle: number, breathe: number, time: number) {
    ctx.strokeStyle = "#1f1308";
    ctx.lineWidth = 3;
    ctx.fillStyle = "#facc15";
    ctx.beginPath();
    ctx.roundRect(x - 16, y - 13, 32, 29 + breathe, 9);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = "#f97316";
    ctx.beginPath();
    ctx.moveTo(x - 20, y - 8);
    ctx.lineTo(x - 31 - wiggle, y - 17);
    ctx.lineTo(x - 25, y - 2);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
  ...
  private drawRockturtle(ctx: CanvasRenderingContext2D, x: number, y: number, wiggle: number, breathe: number) {
    ctx.strokeStyle = "#1f2937";
    ctx.lineWidth = 3;
    ctx.fillStyle = "#64748b";
    ctx.beginPath();
    ctx.ellipse(x, y, 24, 16 + breathe * 0.3, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = "#475569";
    for (let index = -1; index <= 1; index += 1) {
      ctx.beginPath();
      ctx.arc(x + index * 10, y - 5 + Math.abs(index) * 3, 6, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
    }
    ctx.fillStyle = "#94a3b8";
    ctx.beginPath();
    ctx.roundRect(x + 13, y - 8 + wiggle * 0.2, 15, 15, 5);
    ctx.fill();
    ctx.stroke();
    this.drawEyes(ctx, x + 20, y - 2);
  }

  private drawMoleminer(ctx: CanvasRenderingContext2D, x: number, y: number, wiggle: number, breathe: number) {
    ctx.strokeStyle = "#1f1308";
    ctx.lineWidth = 3;
    ctx.fillStyle = "#92400e";
    ctx.beginPath();
    ctx.roundRect(x - 18, y - 12, 36, 29 + breathe, 13);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = "#facc15";
    ctx.beginPath();
    ctx.moveTo(x - 10, y - 16);
    ctx.lineTo(x, y - 30 - Math.abs(wiggle));
    ctx.lineTo(x + 10, y - 16);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = "#fef3c7";
    ctx.fillRect(x - 20 - wiggle, y + 2, 8, 5);
    ctx.fillRect(x + 12 + wiggle, y + 2, 8, 5);
    this.drawEyes(ctx, x, y - 2);
  }

  private drawMossboar(ctx: CanvasRenderingContext2D, x: number, y: number, wiggle: number, breathe: number) {
    ctx.strokeStyle = "#1f1308";
    ctx.lineWidth = 3;
    ctx.fillStyle = "#166534";
    ctx.beginPath();
    ctx.ellipse(x, y, 25, 16 + breathe * 0.3, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = "#84cc16";
    ctx.beginPath();
    ctx.arc(x - 8, y - 14, 7, 0, Math.PI * 2);
    ctx.arc(x + 4, y - 17, 6, 0, Math.PI * 2);
    ctx.arc(x + 14, y - 12, 5, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#fef3c7";
    ctx.beginPath();
    ctx.moveTo(x - 14, y + 4);
    ctx.lineTo(x - 24 - wiggle, y + 10);
  ...
  private drawGenericCreature(ctx: CanvasRenderingContext2D, x: number, y: number, fill: string, breathe: number) {
    ctx.strokeStyle = "#1f1308";
    ctx.lineWidth = 3;
    ctx.fillStyle = fill;
    ctx.beginPath();
    ctx.roundRect(x - 15, y - 13, 30, 27 + breathe, 9);
    ctx.fill();
    ctx.stroke();
    this.drawEyes(ctx, x, y - 3);
  }

  private drawEyes(ctx: CanvasRenderingContext2D, x: number, y: number) {
