...
function applySceneRegex(regex, replacement, label) {
  const result = patchRegex(scene, regex, replacement, label);
  scene = result.text;
  sceneChanged ||= result.changed;
}
...
const methodBlock = `  private carveNightLight(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number) {
  ...
  private drawNightCanvasMask(ctx: CanvasRenderingContext2D, width: number, height: number, cameraX: number, cameraY: number, viewport: ViewportBounds) {
    ...
    ctx.save();
    ctx.fillStyle = "rgba(2, 4, 13, 0.92)";
...
.night-field-overlay, .night-player-light { display: none !important; }
@media (max-width: 720px) { .hud-day-night-toggle { left: calc(88px + var(--safe-left)); top: calc(8px + var(--safe-top)); min-height: 34px; padding: 5px 9px; border-width: 1px; font-size: 10px; } }
