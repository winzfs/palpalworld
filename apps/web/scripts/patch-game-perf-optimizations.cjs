...
// Inject perf cache fields right before onInputChange declaration
replaceIn("scene",
...
// Replace resize handler to cache rect dimensions
replaceIn("scene",
...
// Cache getCameraOffset / getViewportBounds / screenToWorld
replaceIn("scene",
...
// Replace draw() to use cached width/height
replaceIn("scene",
...
// Throttle world_snapshot dispatch + mutate maps instead of recreating
replaceIn("scene",
...
// Reuse previousPlayerPositions object instead of cloning every frame
replaceIn("scene",
...
// Cache getSceneBuildings result and invalidate when buildings change
replaceIn("scene",
...
// Invalidate buildings cache on dismantle / remote update
replaceIn("scene",
...
// Use a typed Iterable for batch dismantle (skipping syncDelete loop in scene)
replaceIn("scene",
...
// Ensure removeBuildParts is imported
replaceIn("scene",
...
// Add cached mounted-pet speed helpers right after getMountedPlayerMoveSpeed
replaceIn("client",
...
// Add cached refs alongside existing refs
replaceIn("client",
...
// Add listener effect that caches build parts and invalidates mount speed
replaceIn("client",
...
// Replace movement tick to use cached parts, cached mount speed, merged attack tick
replaceIn("client",
...
// Merge attack input into main tick (insert call before applyDemoSnapshot)
replaceIn("client",
...
// Remove the separate attack rAF effect (replace with ref-sync effect)
replaceIn("client",
